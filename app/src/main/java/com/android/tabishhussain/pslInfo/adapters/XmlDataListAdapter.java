...
/**
 * Created by tabish on 1/22/16.
 */
public class XmlDataListAdapter extends BaseAdapter {
    ...
    Context context;
    ViewHolder mHolder;
    List<String> pslTeams;

    public XmlDataListAdapter(Context context) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (!TextUtils.isEmpty(matchStatus.battingTeam)) {
            ...
            mHolder.label_batsman.setText("Note :");
            mHolder.batsman1.setText("This information is currently not available. will be available soon");
        } else {
            setVisibility(mHolder, View.INVISIBLE);
            mHolder.startingTime.setText(matchStatus.si);
    ...
    public void TypeFilter(int position) {
        switch (position) {
            ...
            case 3:
                ...
                for (MatchStatus matchStatus :
                        mCurrentData.AllMatchStatus) {
                    if (!TextUtils.isEmpty(matchStatus.matchOverStatement)) {
                        mTypeFilteredData.AllMatchStatus.add(matchStatus);
                    }
                }
                dayFilter(2);
    ...
    public boolean isPSLMatch(MatchStatus matchStatus) {
        pslTeams = new ArrayList<>();
        pslTeams = Arrays.asList(context.getResources().getStringArray(R.array.psl_teams));
        for (int i = 0; i < pslTeams.size(); i++) {
            if (matchStatus.teamA.toLowerCase(Locale.US).contains(pslTeams.get(i))
                    || matchStatus.teamB.toLowerCase(Locale.US).contains(pslTeams.get(i))) {
                return true;
            }
        }
        return false;
    ...
    public void setVisibility(ViewHolder mHolder, int visibility) {
        ...
        mHolder.overs.setVisibility(View.INVISIBLE);
        mHolder.battingWickets.setVisibility(visibility);
        mHolder.label_batsman.setVisibility(visibility);
        mHolder.label_bowler.setVisibility(View.INVISIBLE);
        mHolder.batsman1.setVisibility(visibility);
        mHolder.batsman2.setVisibility(View.GONE);
        mHolder.bowler.setVisibility(View.INVISIBLE);
